...
func ValidateBuildRepository(ctx context.Context, repo string, change spec.ChangeContract) error {
	...
	if objectFormat != want.GitObjectFormat {
		return fmt.Errorf("baseline git_object_format mismatch: got %s want %s", objectFormat, want.GitObjectFormat)
	}
	baseTree, err := gitutil.Output(ctx, root, nil, nil, "rev-parse", want.BaseCommit+"^{tree}")
	if err != nil {
		return fmt.Errorf("resolve locked baseline tree: %w", err)
	}
	if baseTree != want.BaseTree {
		return fmt.Errorf("baseline base_tree mismatch: got %s want %s", baseTree, want.BaseTree)
	}
	specificationSum, err := change.Specification.SHA256()
	if err != nil {
		return fmt.Errorf("hash specification: %w", err)
	}
	...
	if err := cmd.Run(); err != nil {
		...
		if errors.As(err, &exitErr) && exitErr.ExitCode() == 1 {
			return fmt.Errorf("baseline base_commit %s is not an ancestor of producer HEAD", want.BaseCommit)
		}
		return fmt.Errorf("validate baseline ancestry: %w", err)
	}
	return nil
}
