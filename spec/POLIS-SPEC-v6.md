...
`verify` MUST validate v6 inventory, checksums, manifest digest, plan schema, exact packaged Change Contract digest, baseline, strategy, references, path containment, graph, and consistency with existing proof/evidence before exposing the plan. `inspect` MUST report plan presence, schema, strategy, step count, and requirement-to-plan-to-proof traceability. Its JSON output MUST separate REQ/AC-linked test and implementation step IDs; proof references MUST include the IDs and kinds of plan steps that declare the linked contract proof. Historical and unplanned artifacts MUST report plan absence. `preflight` and `apply` MUST inherit this package-verification boundary and reject an invalid planned package before consumer mutation. Formats v2-v5 retain their historical exact inventory and verification semantics.
