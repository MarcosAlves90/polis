...
```bash
./scripts/github-release.sh --tag v6.1.0
```

Set `POLIS_GH` when a specific GitHub CLI should be the default for the shell/session:

```bash
POLIS_GH="$HOME/tools/gh" ./scripts/github-release.sh --tag v6.1.0
```
...
```bash
./scripts/github-release.sh --tag v6.1.0 --publish
```
...
```bash
./scripts/github-release.sh --tag v6.1.0 --remote upstream
```
