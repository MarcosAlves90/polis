...
```bash
./scripts/github-release.sh --tag v6.4.0
```

Set `POLIS_GH` when a specific GitHub CLI should be the default for the shell/session:

```bash
POLIS_GH="$HOME/tools/gh" ./scripts/github-release.sh --tag v6.4.0
```
...
```bash
POLIS_GO=/opt/go/bin/go ./scripts/github-release.sh --tag v6.4.0
./scripts/github-release.sh --tag v6.4.0 --go /opt/go/bin/go
```
...
```bash
./scripts/github-release.sh --tag v6.4.0 --publish
```
...
```text
polis-v6.4.0-offline-<GOOS>-<GOARCH>.zip
```
...
```bash
./scripts/github-release.sh --tag v6.4.0 --remote upstream
```
