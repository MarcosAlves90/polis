...
```bash
./scripts/github-release.sh --tag v6.8.1
```

Set `POLIS_GH` when a specific GitHub CLI should be the default for the shell/session:

```bash
POLIS_GH="$HOME/tools/gh" ./scripts/github-release.sh --tag v6.8.1
```
...
```bash
POLIS_GO=/opt/go/bin/go ./scripts/github-release.sh --tag v6.8.1
./scripts/github-release.sh --tag v6.8.1 --go /opt/go/bin/go
```
...
```bash
./scripts/github-release.sh --tag v6.8.1 --publish
```
...
```text
polis-v6.8.1-offline-<GOOS>-<GOARCH>.zip
```
...
```bash
./scripts/github-release.sh --tag v6.8.1 --remote upstream
```
